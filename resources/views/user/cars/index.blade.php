            ...
            @foreach($cars as $car)
                <div class="card-wrapper relative">
                    <div class="card bg-white rounded-xl shadow overflow-hidden">
                        <img src="{{ $car->image_url ?? asset('images/no-car.png') }}" alt="{{ $car->name }}" class="h-48 w-full object-cover object-center">
                        <div class="card-body p-5 space-y-3">
                            ...
                            <div class="flex justify-between items-center pt-2 space-x-4 mt-4">
                                <a href="https://wa.me/6281234567890?text=Saya%20ingin%20menyewa%20{{ urlencode($car->name) }}" target="_blank" class="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg shadow">
