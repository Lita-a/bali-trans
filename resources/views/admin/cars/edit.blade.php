        ...
        <div class="bg-white shadow rounded-xl p-6 max-w-xl mx-auto">
            <form action="{{ route('admin.cars.update', $car->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $car->name) }}" class="w-full p-3 border rounded" required>
                </div>

                <div>
                    <label for="price" class="block mb-1 font-semibold text-gray-700">Harga</label>
                    <input type="number" name="price" id="price" value="{{ old('price', $car->price) }}" class="w-full p-3 border rounded" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $car->quantity) }}" min="1" class="w-full p-3 border rounded" required>
                </div>
                ...
                <div id="imagePreview" class="mt-4 w-64 mx-auto {{ $car->image_url ? '' : 'hidden' }}">
                    <img id="previewImg" 
                         src="{{ $car->image_url ?? 'https://via.placeholder.com/400x300?text=Preview' }}" 
                         alt="Preview" 
                         class="h-48 w-full object-cover rounded-xl shadow-md">
