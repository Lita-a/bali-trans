...
class CartController extends Controller
{
    private function calculateCart(&$cart)
    {
        ...
        foreach ($cart as &$item) {
            ...
            $pricePerDay = $item['with_driver'] ? $item['driver_price'] : $item['price'];
            $item['days'] = $days;
    ...
    public function add($id, Request $request)
    {
        ...
        if (isset($cart[$car->id])) {
            ...
            $cart[$car->id] = [
                'id' => $car->id,
                'name' => $car->name,
                'image' => $car->image ? asset('storage/' . $car->image) : asset('images/no-car.png'),
                'price' => $car->price,
        ...
        session(['cart' => $cart]);
        if ($request->ajax()) {
            return response()->json(['cart' => $cart, 'total' => $this->calculateCart($cart)]);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $cart[$id]['qty'] = $request->qty ?? $cart[$id]['qty'];
        session(['cart' => $cart]);
        if ($request->ajax()) {
            return response()->json(['cart' => $cart, 'total' => $this->calculateCart($cart)]);
        }
    ...
    public function storeOrder(Request $request)
    {
        ...
        $invoicePath = 'storage/invoice_' . $order->id . '.pdf';
        $pdf = PDF::loadView('user.carts.invoice', [
            'order' => $order,
            'cart'  => $cart,
            'totalPrice' => $totalPrice
        ])->save(public_path($invoicePath));

        Mail::to('admin@domain.com')->send(new NewOrderAdminMail($order));
        session()->forget('cart');
